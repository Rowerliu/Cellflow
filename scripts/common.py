...
def read_classifier(args, log_dir):
    classifier_path, _ = get_latest_model_path_in_directory(log_dir)
    logger.log(f"Classifier path: {classifier_path}")
    classifier = create_classifier(**args_to_dict(args, classifier_defaults().keys()))
    classifier.load_state_dict(dist_util.load_state_dict(classifier_path, map_location="cpu"), strict=False)  # classifier path
