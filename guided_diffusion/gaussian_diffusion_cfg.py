...
class GaussianDiffusion:
    ...
    def condition_cfg(self, model, p_mean_var, x, t, weight, model_kwargs=None):
        """
        ...
        eps_cond = model(x, self._scale_timesteps(t), **dict_)
        # null embedding
        dict_['null'] = True
        eps_uncond = model(x, self._scale_timesteps(t), **dict_)

        eps = (1+weight) * eps_cond - weight * eps_uncond
        if x.shape != eps.shape:  # if learn_sigma is True: eps.
            B, C = x.shape[:2]
            assert eps.shape == (B, C * 2, *x.shape[2:])
    ...
    def ddim_sample(
        ...
        if cond_fn is not None and weight == -1:
            out = self.condition_score(cond_fn, out, x, t, model_kwargs=model_kwargs)
        elif weight >= 0:
            out = self.condition_cfg(model, out, x, t, weight, model_kwargs=model_kwargs)
    ...
    def fourier_frequency_list_loop(
        ...
        # out_fourier_magnitude_list.append(image_fourier_trans(image))
        for sample in self.ddim_reverse_sample_loop_progressive(
                model,
                image,
                clip_denoised=clip_denoised,
