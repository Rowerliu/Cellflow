...
def main():
    ...
    folder = os.path.join(rf'D:\01_LZY\02_Code\08_process-learning-V3\01_results\20240313_ROSE_TTFF_s500\neg2pos')
    ...
    source_dir = r'D:\01_LZY\02_Code\01_guided-diffusion\02_results_saved\20231025_ROSE_r384-256_Blur_10w\Negative\False\model'
    source_model, diffusion = read_model_and_diffusion(args_source, source_dir)

    target_dir = r'D:\01_LZY\02_Code\01_guided-diffusion\02_results_saved\20231025_ROSE_r384-256_Blur_10w\Positive\True\model'
    ...
    for k, (source, extra) in enumerate(data):
        ...
        if k < lens//args.batch_size:
            ...
            if args.timestep_respacing is not None:
                diffusion_steps = int(args.timestep_respacing.split('ddim')[1])
...
def create_argparser(type=None):
    defaults = dict(
        data_dir=r"D:\01_LZY\01_Data\03_Pathology\01_ROSE\02_resize\ROSE_500\Negative",
        image_size=256,
        batch_size=1,  # depends on the size of GPU
    )
    ...
    parser.add_argument(
            ...
            type=int,
            default=1,
            help="Source dataset ROSE Pos."
        )
    parser.add_argument(
            "--target",
            type=int,
            default=0,
            help="Target dataset ROSE Neg."
    )
