...
def main():
    ...
    folder = os.path.join(rf'F:\BUAA\02_Code\02_ZhangLab\08_process-learning-V2\01_results\20231101_ROSE_FFFF')
    pathlib.Path(folder).mkdir(parents=True, exist_ok=True)

    time_start = datetime.datetime.now().strftime('%Y-%m-%d %H-%M-%S')
    logger.configure(dir=folder, log_suffix=time_start)

    logger.log("starting to synthesis data.")
    ...
    source_dir = r'F:\BUAA\02_Code\02_ZhangLab\01_guided-diffusion-lzy\02_results_saved\20231028_ROSE_Neg_Blur_384-256\False\model'
    source_model, diffusion = read_model_and_diffusion(args_source, source_dir, synthetic=False)

    target_dir = r'F:\BUAA\02_Code\02_ZhangLab\01_guided-diffusion-lzy\02_results_saved\20231028_ROSE_Pos_Blur_384-256\False\model'
    ...
    for k, (source, extra) in enumerate(data):
        ...
        if k < lens//args.batch_size:
            ...
            diffway_list = [0]  # initial time step zero for diffway_start
            diffway_list = diffway_list + get_adaptive_list(out_fourier_magnitude_list, amount)
...
def create_argparser(type=None):
    defaults = dict(
        data_dir=r"G:\12_Data\03_Pathology\08_Process-Learning\01_ROSE\02_test\Negative_256_8",
        image_size=256,
        batch_size=1,
    )
