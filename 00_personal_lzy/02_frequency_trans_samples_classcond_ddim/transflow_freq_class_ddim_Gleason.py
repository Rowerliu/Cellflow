...
def main():
    ...
    folder = r'D:\lzy\03_code\04_Cellflow\02_Flow\01_result\20250607_Gleason\FFTT_ddim100_a10_classifier'  # todo
    ...
    source_dir = r'D:\lzy\03_code\04_Cellflow\01_Diffusion\01_result\04_Gleason_cls_512_Blur_10w\False'
    source_model, diffusion = read_model_and_diffusion(args_source, source_dir)

    target_dir = r'D:\lzy\03_code\04_Cellflow\01_Diffusion\01_result\04_Gleason_cls_512_Blur_10w\True'
    target_model, _ = read_model_and_diffusion(args_target, target_dir)

    classifier_dir = r'D:\lzy\03_code\04_Cellflow\01_Diffusion\01_result\04_Gleason_cls_512_Blur_10w\Classifier'
        ...
        if bs < lens//args.batch_size:
            if int(extra["y"]) == args.source:
                ...
                while m < amount:
                    ...
                    prefix = source_domain + '_' + target_domain
    ...
    grouped_latents = np.reshape(latents, (bs, amount, latents.shape[1], latents.shape[2], latents.shape[3]))
    latents_path = os.path.join(image_subfolder, 'latent.npy')
    np.save(latents_path, grouped_latents)

    targets = np.concatenate(targets, axis=0)
    grouped_targets = np.reshape(targets, (bs, amount, targets.shape[1], targets.shape[2], targets.shape[3]))
...
def save_tensor_as_images(tensor, save_dir, prefix, batch_idx, step_idx=None):
    ...
    for i in range(batch_size):
        ...
        if step_idx is not None:
            filename = f"{prefix}_bs{batch_idx:04d}_img{i:04d}_step{step_idx:04d}.jpg"
        else:
            filename = f"{prefix}_bs{batch_idx:04d}_img{i:04d}.jpg"
...
def create_argparser(type=None):
    defaults_all = dict()
    defaults = dict(
        data_dir=r"E:\01_LZY\01_Data\z12_Gleason\02_resize\r320",  # todo
        clip_denoised=False,
        batch_size=1,
        ...
        image_size=256,
        class_cond=True,
        num_classes=4,  # dataset classes
        out_channels=4,  # dataset classes
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Source dataset Gleason."
    )
    parser.add_argument(
        "--target",
        type=int,
        default=2,
        help="Target dataset Gleason."
    )
    parser.add_argument(
        "--domain_name",
        type=list,
